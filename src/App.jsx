...
import {
  ...
  Send, Loader2, ShieldCheck, AlertCircle, RotateCcw,
} from 'lucide-react';
...
const DURATION = 7;
const POINTS_PER_TAKA = 3.2;
const POINTS_PER_AD = 1;
const MIN_TAKA = 20;

const AD_POOL = [
  { id: 'ad-01', sponsor: 'Doyel Mart', tagline: 'Daily essentials, fair prices', hue: '#2E6F52' },
  { id: 'ad-02', sponsor: 'Nodi Telecom', tagline: 'New data packs from \u09F319', hue: '#4C6B8A' },
  { id: 'ad-03', sponsor: 'Shohoj Ride', tagline: 'Book a ride in seconds', hue: '#B23A2E' },
  { id: 'ad-04', sponsor: 'Ranna Ghar', tagline: 'This week\u2019s kitchen deals', hue: '#5A7D5A' },
  { id: 'ad-05', sponsor: 'Projapoti Beauty', tagline: 'Skincare picks for summer', hue: '#A8536B' },
  { id: 'ad-06', sponsor: 'Moyna Bank', tagline: 'Save smarter this year', hue: '#3E6B5C' },
  { id: 'ad-07', sponsor: 'Turag Travels', tagline: 'Weekend getaways from \u09F3990', hue: '#8A6BAF' },
  { id: 'ad-08', sponsor: 'Bijoy Electronics', tagline: 'Gadget deals, today only', hue: '#D4A94A' },
  { id: 'ad-09', sponsor: 'Green Delta Grocers', tagline: 'Fresh, delivered at your door', hue: '#6B8F5A' },
  { id: 'ad-10', sponsor: 'Amar Broadband', tagline: 'Faster home internet', hue: '#6B7FA3' },
];
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="text-xs font-body mt-3 text-center" style={{ color: C.textMuted }}>
            {stage === 'verified'
              ...
              : `Keep this open for ${DURATION}s to earn ${POINTS_PER_AD} point`}
          </div>
        </div>
      </div>
    </div>
...
export default function PoyshaApp() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const [pts, w, wd] = await Promise.all([
  ...
  const handleReset = async () => {
    await Promise.all([saveKey('points', 0), saveKey('watched-ads', []), saveKey('withdrawals', [])]);
    setPoints(0);
    setWatched([]);
    setWithdrawals([]);
  ...
  const availableAds = AD_POOL.filter((a) => !watchedIds.has(a.id));
              ...
              {availableAds.length === 0 ? (
                ...
                >
                  ...
                  <div className="text-xs" style={{ color: C.textMuted }}>
                    You&#8217;ve watched every ad available right now. New ones arrive regularly \u2014 check back soon.
                  </div>
                </div>
              ) : (
